...
- `any` is repo-wide `warn` in `.eslintrc.json`, but ratcheted up to `error` for specific files/folders confirmed fully typed (see the `overrides` block) — currently `src/components/Panels/Jog/**` and a handful of `src/Services/*.ts` collaborators. Extend that list (rather than loosening it) as more of the codebase gets fully typed; most of `src/` still has legitimate untyped `any` usage (dynamic settings-tree values, extension-message payloads, etc.) so don't assume a whole top-level folder is clean without checking.
