/*
formatHelper.ts - ESP3D WebUI helper file
...
const generateFormattedSelectOptionList = (rawOpt: Array<{ [key: string]: string }>): SelectOption[] => {
...
const formatSettingItem = (settingItem: RawSettingItem): SettingFieldProps => {
    const { P, T, V, H, O: options, M, S, MS, R, U, E } = settingItem
    let settingFieldProps: SettingFieldProps = {
        id: P,
        initial: V,
        label: H,
        type: T,
...
const formatStructure = (settings: RawSettingItem[]): FeaturesStructure =>
