/*
 Input.tsx - ESP3D WebUI component file
...
const Reveal: FunctionalComponent<RevealProps> = ({ applyTo }) => {
    ...
    return (
        <div class="form-icon passwordReveal" onClick={clickReveal}>
            {reveal ? (
                ...
                <Eye style={{ width: "1rem", height: "1rem" }} class="has-error icon-reveal" />
            )}
        </div>
    )
}

const ClearText: FunctionalComponent<ClearTextProps> = ({ setValue }) => {
    const clickClear = () => {
        useUiContextFn.haptic()
    ...
    return (
        <div class="form-icon clearShortkey" onClick={clickClear}>
            <XCircle style={{ width: "1rem", height: "1rem" }} class="icon-clear" />
        </div>
    )
}
...
const Input: FunctionalComponent<InputProps> = ({
    ...
    let ScanNetworks: (() => void) | null = null
    const refreshList = () => {
                    ...
                    onClick={(e: TargetedMouseEvent<HTMLButtonElement>) => {
                        ...
                        showModal({
                            ...
                            content: (
                                <ScanApList
                                    id={modalId}
                                    setValue={(value: string) => setValue && setValue(value)}
                                    refreshfn={(scannetwork: () => void) =>
